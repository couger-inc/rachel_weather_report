...
class Infrastructure {
  ...
  get nowIcon() {
    return this._now.icon;
  }
  get nowTemp() {
    return this._now.temperature;
  }

  static sendBody(connection, speechMessage, icon) {
...
class Darksky {
   ...
   */
  static generateSpeechMessage(nowIcon, high, low, current) {
    const iconJa = this.mapIconsJa(nowIcon);
    // eslint-disable-next-line max-len
    return `只今の天気は、${iconJa}。予想最高気温は、${high.toFixed()}度。最低気温は、${low.toFixed()}度。現在の気温は、${current.toFixed()}度です。`;
  }
